...
num = 0
count = 0
while count < n: # n개를 찾을 떄까지 실행시킨다.
    temp_str = str(num) # num을 문자열로 만든다.
    temp_list = list(temp_str) # num을 한 문자씩 잘라서 list로 만든다.
    #print('temp_list: ', temp_list)
    l = len(temp_list)  # 5
    for i in range(0, l-2): # 3 번 = 5 -2, i = 0, 1, 2 
        temp_3_list = temp_list[i:i+3] # 3개씩 잘라서 만든 list
        #print('temp_3_list: ', temp_3_list)
        if temp_3_list == ['6', '6', '6']:
            count += 1
            break
    if count == n:
        break

    num += 1 # num을 1 증가시켜서 다시 확인한다.
print(num)
