...
for i in range(C):
    ...
    # cache = [ [-1]*(s+1) ]*(len(input_list) + 1) 
    # cache[i][j] = "i번째 인덱스부터 시작하는 부분 수열, j개로 나눌 수 있는 경우" 의 최소 오차 제곱의 합.
    cache = [[-1]*11 for _ in range(len(input_list) + 1)]
    # Python에서 list를 선언할 때에 * 을 잘못쓰면 포인터로 입력되서 리스트가 이상하게 처리될 수 있음.
    # 이럴 때를 방지하기 위해 위와 같이 for문을 이용하여 되도록 상수로 선언한다.
