...
def isInsideList(the_list, the_element):
    if the_element in the_list:
        print('the_list:',the_list,'the_element:',the_element,'is in the list')
        return True
    else:
        print('the_list:',the_list,'the_element:',the_element,'not in the list')
        return False
    ...
    if len(list_of_couples_remain) == 1:
        ...
        for couple in list_of_couples_remain:
            ...
            for other_couple in list_of_couples_remain: # 뽑힌 커플이 속한 다른 커플을 모두 삭제한다.
                ...
                print('---------이번에 확인할 couple은 ',other_couple)
                if isInsideList(other_couple, a) or isInsideList(other_couple, b):
                    print('removing this element: ', other_couple)
                    couple_to_remove.append(other_couple)
                    # list_of_couples_remain.remove(other_couple)
                else:
                    print(other_couple, '는 제거되지 않습니다.')
            for couple in couple_to_remove:
                list_of_couples_remain.remove(couple)
...
# print('couples_listed: ')
# for i in couples_listed:
#     for j in i:
#         print(j)
...
for i in range(TEST_CASES):
    ...
    print(i+1,'번째 case의 결과: ', searching_couple_cases(couples_listed[i], 0))
